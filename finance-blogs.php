<?php require "header.php" ?>

        <div class="container-fluid bg-primary py-5 bg-header" style="margin-bottom: 90px;">
            <div class="row py-5">
                <div class="col-12 pt-lg-5 mt-lg-5 text-center">
                    <h1 class="display-4 text-white animated zoomIn">Finance Blogs</h1>
                    <a href="" class="h5 text-white">Home</a>
                    <i class="far fa-circle text-white px-2"></i>
                    <a href="" class="h5 text-white">Finance Blogs</a>
                </div>
            </div>
        </div>
    </div>







    <div class="container-fluid py-5 wow fadeInUp" data-wow-delay="0.1s">
        <div class="container py-5">
            <div class="section-title text-center position-relative pb-3 mb-5 mx-auto" style="max-width: 600px;">
                <h5 class="fw-bold text-primary text-uppercase">Latest Blog</h5>
                <h1 class="mb-0">Read Articles from Our Post</h1>
            </div>
            <div class="row g-5">
                <div class="col-lg-3 wow slideInUp" data-wow-delay="0.3s">
                    <div class="blog-item bg-light rounded overflow-hidden">
                        <div class="blog-img position-relative overflow-hidden">
                            <img class="img-fluid" src="blog/real-estate/estate.jpeg" alt="">
                            <!-- <a class="position-absolute top-0 start-0 bg-primary text-white rounded-end mt-5 py-2 px-4" href="business-blogs.html">Business Blogs</a> -->
                        </div>
                        <div class="p-4">
                            <div class="d-flex mb-3">
                                <small class="me-3"><i class="far fa-user text-primary me-2"></i>Nathan Isaac</small>
                                <small><i class="far fa-calendar-alt text-primary me-2"></i>25 Jan, 2023</small>
                            </div>
                            <h4 class="mb-3">Why Real Estate</h4>
                            <!-- <p>Dolor et eos labore stet justo sed est sed sed sed dolor stet amet</p> -->
                            <a class="text-uppercase" href="business-blogs.html">Read More <i class="bi bi-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 wow slideInUp" data-wow-delay="0.6s">
                    <div class="blog-item bg-light rounded overflow-hidden">
                        <div class="blog-img position-relative overflow-hidden">
                            <img class="img-fluid" src="blog/real-estate/spending-money.jpeg" alt="">
                            <!-- <a class="position-absolute top-0 start-0 bg-primary text-white rounded-end mt-5 py-2 px-4" href="finance-blogs.html">Finance Blog</a> -->
                        </div>
                        <div class="p-4">
                            <div class="d-flex mb-3">
                                <small class="me-3"><i class="far fa-user text-primary me-2"></i>Nathan Isaac</small>
                                <small><i class="far fa-calendar-alt text-primary me-2"></i>25 Jan, 2023</small>
                            </div>
                            <h4 class="mb-3">Spending Your Money</h4>
                            <p>I have no right to tell you what to spend your money on; no right at ...</p>
                            <a class="text-uppercase" href="finance-blogs.html">Read More <i class="bi bi-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 wow slideInUp" data-wow-delay="0.9s">
                    <div class="blog-item bg-light rounded overflow-hidden">
                        <div class="blog-img position-relative overflow-hidden">
                            <img class="img-fluid" src="blog/finance/wealth.jpeg" alt="">
                            <!-- <a class="position-absolute top-0 start-0 bg-primary text-white rounded-end mt-5 py-2 px-4" href="faith-blogs.html">Faith Blogs</a> -->
                        </div>
                        <div class="p-4">
                            <div class="d-flex mb-3">
                                <small class="me-3"><i class="far fa-user text-primary me-2"></i>Nathan Isaac</small>
                                <small><i class="far fa-calendar-alt text-primary me-2"></i>23 Jan, 2023</small>
                            </div>
                            <h4 class="mb-3">Wealth and Riches</h4>
                            <p>Do not be lured into displaying riches at the expense of being wealthy.</p>
                            <a class="text-uppercase" href="faith-blogs.html">Read More <i class="bi bi-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 wow slideInUp" data-wow-delay="0.9s">
                    <div class="blog-item bg-light rounded overflow-hidden">
                        <div class="blog-img position-relative overflow-hidden">
                            <img class="img-fluid" src="blog/finance/bedroom.jpeg" alt="">
                            <!-- <a class="position-absolute top-0 start-0 bg-primary text-white rounded-end mt-5 py-2 px-4" href="self-care-blogs.html">Self Help Blogs</a> -->
                        </div>
                        <div class="p-4">
                            <div class="d-flex mb-3">
                                <small class="me-3"><i class="far fa-user text-primary me-2"></i>Nathan Isaac</small>
                                <small><i class="far fa-calendar-alt text-primary me-2"></i>23 Jan, 2023</small>
                            </div>
                            <h4 class="mb-3">Investment and Bedroom Decisions</h4>
                            <p>It's just a short story I'm going to be sharing ...</p>
                            <a class="text-uppercase" href="self-care-blogs.html">Read More <i class="bi bi-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>













     <!-- Footer Start -->
     <?php require "footer.php" ?>